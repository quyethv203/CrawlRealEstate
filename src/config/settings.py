...
class Config:
    ...
    MONGODB_URI: str = os.getenv('MONGODB_URI',
                                 '')
    MONGODB_DATABASE: str = os.getenv('MONGODB_DATABASE', 'real_estate_db')

    # Crawler
    CRAWL_DELAY: int = int(os.getenv('CRAWL_DELAY', '2'))
    LINK_PER_BATCH: int = int(os.getenv('LINK_PER_PATCH', '20'))
    ITEM_PER_BATCH: int = int(os.getenv('ITEM_PER_PATCH', '20'))
    PAGES_SITE: int = int(os.getenv('PAGES_SITE', '50'))  # Số trang tối đa crawl trên mỗi site

    # LLM Configuration
    LLM_BATCH_SIZE: int = int(os.getenv('LLM_BATCH_SIZE', '5'))  # Số properties xử lý cùng lúc với LLM
    LLM_ENABLED: bool = os.getenv('LLM_ENABLED', 'True').lower() == 'true'
    LLM_PROVIDER: str = os.getenv('LLM_PROVIDER', 'gemini/gemini-2.0-flash')
